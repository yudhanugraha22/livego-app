#!/bin/bash
# ===============================================
# LIVEGO MASTER STARTER - BUILD #119 (ENTERPRISE)
# Full Modular | HMAC V2 | Riverpod | Cache Ready
# ===============================================
cd ~/livego_project
rm -rf lib && mkdir -p lib/core lib/providers lib/ui/mobile lib/ui/tv/widgets lib/ui/player lib/ui/shared

echo "1. Membangun Jantung Data (HMAC V2)..."
...
echo "3. Membangun UI shared & Kursor TV..."
...
echo "4. Membangun UI Mobile (CineFlow Identik)..."
...
class MobileHome extends ConsumerWidget {
  ...
  @override Widget build(BuildContext context, WidgetRef ref) {
    ...
    return Scaffold(
      ...
      appBar: AppBar(backgroundColor: const Color(0xFF05070D), elevation: 0, title: const Text("LiveGo", style: TextStyle(fontWeight: FontWeight.bold, color: Color(0xFF00D9FF)))),
      body: SingleChildScrollView(child: Column(children: [
        bn.when(data: (d)=>d==null?const SizedBox():_bannerUI(context, d, selP), loading: ()=>const SizedBox(height: 180), error: (_,__)=>const SizedBox()),
        const Divider(color: Colors.white10, indent: 15, endIndent: 15),
        _chips(ref, ["Melolo","FreeReels","FlickReels","RapidTV"], selP, true),
        _chips(ref, ["dub-indo","popular","trending","new-release"], selC, false),
  ...
  Widget _chips(WidgetRef ref, List<String> l, String s, bool isP) => SizedBox(height: 45, child: ListView.builder(scrollDirection: Axis.horizontal, padding: const EdgeInsets.only(left: 15), itemCount: l.length, itemBuilder: (ctx, i)=>Padding(padding: const EdgeInsets.only(right: 8), child: ActionChip(label: Text(l[i]), backgroundColor: s.toLowerCase()==l[i].toLowerCase()||s==l[i]?(isP?const Color(0xFF8B5CF6):const Color(0xFF00D9FF)):Colors.white10, onPressed: ()=>isP?ref.read(platformProvider.notifier).state=l[i].toLowerCase():ref.read(categoryProvider.notifier).state=l[i]))));
}
EOF

echo "5. Membangun UI TV (Sidebar + 7 Column Grid)..."
...
echo "6. Membangun Player TV & Main Switcher..."
...
class _LiveGoPlayerState extends State<LiveGoPlayer> {
  ...
  @override Widget build(BuildContext context) {
    ...
    return Scaffold(backgroundColor: Colors.black, body: GestureDetector(onTap: (){ setState(()=>ui=!ui); if(ui) _startT(); }, child: Stack(children: [
      ...
      if (ui && ready) Positioned(bottom: 30, left: isT?60:20, right: isT?60:20, child: Container(padding: const EdgeInsets.all(20), decoration: BoxDecoration(color: const Color(0xFF0D2A4F).withOpacity(0.9), borderRadius: BorderRadius.circular(30), border: Border.all(color: const Color(0xFF00D9FF).withOpacity(0.3))), child: Column(mainAxisSize: MainAxisSize.min, children: [VideoProgressIndicator(_v!, allowScrubbing: true, colors: const VideoProgressColors(playedColor: Colors.redAccent)), const SizedBox(height: 15), Row(mainAxisAlignment: MainAxisAlignment.spaceEvenly, children: const [Icon(Icons.skip_previous), Icon(Icons.play_arrow, size: 40), Icon(Icons.skip_next), Text("AUTO"), Icon(Icons.list)])]))),
    ])));
  }
}
EOF
...
echo "Pushing Final Master Build to GitHub..."
git add . && git commit -m "Build 119 FINAL: Enterprise Architecture with Starter Plan Support" && git push origin main --force
echo "--- SEMUA PROSES BERHASIL. CEK GITHUB ACTIONS ---"
