...
class _HomePageState extends State<HomePage> {
  ...
  _init() async {
    ...
    activeApis = all.where((a)=>p.getBool('api_$a')??["Melolo","FreeReels","FlickReels","RapidTV"].contains(a)).toList();
  ...
  _fetch() async {
    ...
    String p = (selectedCategory == "Dubbing") ? "/api/v2/search?category_p=$selectedPlatform&q=sulih suara&lang=id" : "/api/v2/home?category_p=$selectedPlatform&lang=id";
  ...
  @override Widget build(BuildContext context) {
    ...
    return Scaffold(backgroundColor: const Color(0xFF0D1117), appBar: AppBar(backgroundColor: const Color(0xFF161B22), elevation: 0, title: const Text("Livego", style: TextStyle(color: Colors.blueAccent, fontWeight: FontWeight.bold))),
      body: SingleChildScrollView(child: Column(children: [
        if (banner != null) _buildBanner(),
        _chipList(activeApis, selectedPlatform, (v){ selectedPlatform=v.toLowerCase(); _fetch(); }, const Color(0xFF8B5CF6)),
        const SizedBox(height: 8),
        _chipList(["Dubbing","Populer","New","Trending"], selectedCategory, (v){ selectedCategory=v; _fetch(); }, Colors.blueAccent),
        const SizedBox(height: 10),
        load ? const Center(child: Padding(padding: EdgeInsets.all(50), child: CircularProgressIndicator())) : _buildGrid(isT),
      ])),
    );
  }
