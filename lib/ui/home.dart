...
import 'widgets.dart';
import 'player.dart'; // PENTING: Koneksi agar bisa buka detail film
  ...
  @override Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      body: SingleChildScrollView(child: Column(children: [
        ...
        isLoading ? const Center(child: Padding(padding: EdgeInsets.all(50), child: CircularProgressIndicator())) : GridView.builder(shrinkWrap: true, physics: const NeverScrollableScrollPhysics(), padding: const EdgeInsets.all(15), gridDelegate: SliverGridDelegateWithFixedCrossAxisCount(crossAxisCount: isTV ? 7 : 4, childAspectRatio: 0.65, crossAxisSpacing: 10, mainAxisSpacing: 10), itemCount: dramas.length, itemBuilder: (c, i) => TVButton(onTap: () { 
          // Tombol ini sekarang mengenal DetailPage
          Navigator.push(context, MaterialPageRoute(builder: (ctx) => DetailPage(id: dramas[i]['id'], source: selectedPlatform.toLowerCase()))); 
        }, child: Column(children: [Expanded(child: ClipRRect(borderRadius: BorderRadius.circular(10), child: Image.network(dramas[i]['cover'], fit: BoxFit.cover))), Text(dramas[i]['title'], maxLines: 1, style: const TextStyle(fontSize: 9))]))),
      ])),
    );
  }
