...
class _TVButtonState extends State<TVButton> {
  ...
  @override Widget build(BuildContext context) {
    ...
    }, child: GestureDetector(onTap: widget.onTap, child: AnimatedContainer(duration: const Duration(milliseconds: 150), decoration: BoxDecoration(borderRadius: BorderRadius.circular(widget.borderRadius), border: Border.all(color: _isF ? Colors.cyanAccent : Colors.transparent, width: 3.5), boxShadow: _isF ? [BoxShadow(color: Colors.cyanAccent.withOpacity(0.8), blurRadius: 20)] : []), transform: _isF ? (Matrix4.identity()..scale(1.06)) : Matrix4.identity(), child: widget.child)));
  }
}
