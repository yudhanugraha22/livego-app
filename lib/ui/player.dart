...
class _DetailPageState extends State<DetailPage> {
  ...
  load() async {
    final res = await ApiService.get("/api/v2/detail?category_p=$source&id=$id&lang=id");
  ...
  @override Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: loading ? const Center(child: CircularProgressIndicator()) : ListView(children: [
        ...
        Padding(padding: const EdgeInsets.all(15), child: Text(d!['synopsis'], style: const TextStyle(fontSize: 13))),
        const Padding(padding: EdgeInsets.all(15), child: Text("EPISODE", style: TextStyle(fontWeight: FontWeight.bold))),
        GridView.builder(shrinkWrap: true, physics: const NeverScrollableScrollPhysics(), padding: const EdgeInsets.all(15), gridDelegate: const SliverGridDelegateWithFixedCrossAxisCount(crossAxisCount: 5, mainAxisSpacing: 10, crossAxisSpacing: 10), itemCount: d!['total_episodes'], itemBuilder: (c, i) => TVButton(onTap: () => Navigator.push(context, MaterialPageRoute(builder: (ctx) => PlayerPage(id: id, source: source, ep: (i+1).toString()))), child: Container(color: Colors.white10, alignment: Alignment.center, child: Text("${i+1}"))))
      ]),
    );
  }
}
...
class _PlayerPageState extends State<PlayerPage> {
  ...
  @override void initState() { super.initState(); load(); }
  load() async {
    final res = await ApiService.get("/api/v2/video?category_p=$source&id=$id&chapterId=$ep&lang=id");
