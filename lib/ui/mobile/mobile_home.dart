...
class _MobileHomeState extends State<MobileHome> {
  ...
  _f() async {
    setState(()=>load=true);
    final r = await ApiEngine.request("/api/v2/home?category_p=$plat&lang=id");
    setState((){ if(r!=null) ds = r['data']; load = false; });
  }
  @override Widget build(BuildContext context) {
    return Scaffold(backgroundColor: const Color(0xFF0D1117), appBar: AppBar(title: const Text("Livego Mobile"), backgroundColor: Colors.transparent), body: Column(children: [
      SizedBox(height: 50, child: ListView(scrollDirection: Axis.horizontal, children: ["Melolo","FreeReels","FlickReels","RapidTV"].map((p)=>Padding(padding: const EdgeInsets.symmetric(horizontal: 5), child: ActionChip(label: Text(p), onPressed: (){ plat=p.toLowerCase(); _f(); }))).toList())),
