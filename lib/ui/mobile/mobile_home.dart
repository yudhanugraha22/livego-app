...
class MobileHome extends ConsumerWidget {
  ...
  @override Widget build(BuildContext context, WidgetRef ref) {
    ...
    return Scaffold(backgroundColor: const Color(0xFF05070D), appBar: AppBar(backgroundColor: const Color(0xFF05070D), elevation: 0, title: const Text("LiveGo", style: TextStyle(fontWeight: FontWeight.bold, color: Color(0xFF00D9FF)))), body: SingleChildScrollView(child: Column(children: [
      bn.when(data: (list)=>list.isEmpty?const SizedBox():_slider(context, list, selP), loading: ()=>const SizedBox(height: 180), error: (e,s)=>const SizedBox()),
      ...
      ds.when(data: (list)=>GridView.builder(shrinkWrap: true, physics: const NeverScrollableScrollPhysics(), padding: const EdgeInsets.all(15), gridDelegate: SliverGridDelegateWithFixedCrossAxisCount(crossAxisCount: isT?7:4, childAspectRatio: 0.62, crossAxisSpacing: 10, mainAxisSpacing: 10), itemCount: list.length, itemBuilder: (c,i)=>TVButton(radius: 12, onTap: ()=>Navigator.push(context, MaterialPageRoute(builder: (x)=>LiveGoPlayer(id: list[i]['id'], source: selP, title: list[i]['title']))), child: ClipRRect(borderRadius: BorderRadius.circular(12), child: Image.network(list[i]['cover'], fit: BoxFit.cover)))), loading: ()=>const Center(child: CircularProgressIndicator(color: Color(0xFF00D9FF))), error: (e,s)=>const Center(child: Text("Error API", style: TextStyle(color: Colors.grey))))
    ])));
  }
  Widget _slider(BuildContext ctx, List items, String p) => Container(height: 210, child: PageView.builder(itemCount: items.length, itemBuilder: (c, i) => _b(ctx, items[i], p)));
  Widget _b(BuildContext ctx, Map d, String p) => Container(margin: const EdgeInsets.all(15), decoration: BoxDecoration(borderRadius: BorderRadius.circular(28), border: Border.all(color: Colors.white10)), child: Stack(children: [ClipRRect(borderRadius: BorderRadius.circular(28), child: Image.network(d['cover'], fit: BoxFit.cover, width: double.infinity)), Container(decoration: BoxDecoration(borderRadius: BorderRadius.circular(28), gradient: LinearGradient(begin: Alignment.bottomCenter, colors: [Colors.black, Colors.transparent]))), Padding(padding: const EdgeInsets.all(20), child: Text(d['title'], style: const TextStyle(fontWeight: FontWeight.bold, fontSize: 18)))]));
