...
class _AccountPageState extends State<AccountPage> {
    ...
    return Scaffold(
      ...
      body: ListView(padding: const EdgeInsets.all(15), children: [
        ...
        _card([
          ...
          _item(Icons.favorite, "Favorit", "Drama yang disimpan", () {}),  // Poin 2
        ]),
        const SizedBox(height: 20),
        _label("PENGATURAN SYSTEM"),
        _card([
          _item(Icons.settings, "Navigasi Hardware", nav, _showNav), // Poin 3
          _switch(Icons.image, "Background Poster", bg, (v){ setState(()=>bg=v); _save('bg',v); }), // Poin 4
          _switch(Icons.cached, "Gunakan Cache", cache, (v){ setState(()=>cache=v); _save('cache',v); }), // Poin 5
          _switch(Icons.screen_rotation, "Rotasi Manual", rot, (v){ setState(()=>rot=v); _save('rot',v); }), // Poin 6
          _item(Icons.lock, "Widevine DRM", drm, _showDRM), // Poin 7
          _item(Icons.layers, "Kelola Sumber Data", "Aktifkan 8 API", () {}), // Poin 8
        ]),
        const SizedBox(height: 20),
        _label("DUKUNGAN"),
        _card([
          _item(Icons.update, "Cek Pembaruan", "v1.0.0", () {}), // Poin 9
          _item(Icons.message, "Feedback", "Hubungi Developer", () {}), // Poin 10
        ]),
      ]),
    );
  }
  ...
  Widget _item(IconData i, String t, String s, VoidCallback click) => TVButton(onTap: click, child: ListTile(leading: Icon(i, color: Colors.blueAccent), title: Text(t, style: const TextStyle(fontSize: 14)), subtitle: Text(s, style: const TextStyle(fontSize: 11, color: Colors.grey)), trailing: const Icon(Icons.chevron_right, size: 16)));
